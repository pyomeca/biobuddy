...
class ScaleTool:
    ...
    def scale(self, original_model: BiomechanicalModelReal,
        ...
        if unit == "mm":
            ...
            raise NotImplementedError(f"This unit of the marker position in your .c3d static file is {unit}, which is not implemented yet.")
        ...
        if self.max_marker_movement is None:
            ...
            for marker in self.marker_weightings:
                ...
                if np.linalg.norm(max_position - min_position) > self.max_marker_movement:
                    raise RuntimeError(f"The marker {marker} moves of approximately {np.linalg.norm(max_position - min_position)} m during the static trial, which is above the maximal limit of {self.max_marker_movement} m.")
            return
    ...
    def check_segments(self):
        ...
        for segment_name in self.original_model.segments.keys():
            ...
            if inertia is not None and inertia.mass > 0.1 and segment_name not in self.scaling_segments.keys():
                raise RuntimeError(f"The segment {segment_name} has a positive mass of {self.original_model.segments[segment_name].inertia_parameters.mass}, but is not defined in the scaling configuration.")
    ...
    def scale_model(self, marker_positions: np.ndarray, marker_names: list[str], mass: float) -> BiomechanicalModelReal:
        ...
        for segment_name in self.scaling_segments.keys():
            ...
            scaling_factors[segment_name] = self.scaling_segments[segment_name].compute_scaling_factors(marker_positions, marker_names, original_model_biorbd)
            # Get each segment's scaled mass
            if self.personalize_mass_distribution:
                segment_masses[segment_name] = self.original_model.segments[segment_name].inertia_parameters.mass * scaling_factors[segment_name]
            else:
                segment_masses[segment_name] = self.original_model.segments[segment_name].inertia_parameters.mass * mass/original_mass
        ...
        for segment_name in self.original_model.segments.keys():
            ...
            if segment_name in self.scaling_segments.keys():
                ...
                scaled_model.segments.append(self.scale_segment(self.original_model.segments[segment_name], parent_scale_factor, this_segment_scale_factor, segment_masses[segment_name]))
                ...
                for contact in self.original_model.segments[segment_name].contacts:
                    scaled_model.segments[segment_name].add_contact(self.scale_contact(contact, this_segment_scale_factor))
    ...
    def scale_rt(rt: np.ndarray, scale_factor: np.ndarray) -> np.ndarray:
        rt_matrix = rt.copy()
        rt_matrix[:3, 3] *= scale_factor.reshape(3,)
        return rt_matrix

    def scale_segment(self, original_segment: SegmentReal, parent_scale_factor: np.ndarray, scale_factor: np.ndarray, segment_mass: float) -> SegmentReal:
        """
        ...
        if original_segment.segment_coordinate_system.is_in_global:
            raise NotImplementedError("The segment_coordinate_system is not in the parent reference frame. This is not implemented yet.")
        ...
        original_radii_of_gyration = np.array([np.sqrt(inertia/original_segment.inertia_parameters.mass) for inertia in original_segment.inertia_parameters.inertia])
        ...
        mesh_file = None if original_segment.mesh_file is None else MeshFileReal(
            mesh_file_name=original_segment.mesh_file.mesh_file_name,
            mesh_color=None if original_segment.mesh_file.mesh_color is None else original_segment.mesh_file.mesh_color,
            mesh_scale=None if original_segment.mesh_file.mesh_scale is None else original_segment.mesh_file.mesh_scale * scale_factor,
            mesh_rotation=None if original_segment.mesh_file.mesh_rotation is None else original_segment.mesh_file.mesh_rotation,
            mesh_translation=None if original_segment.mesh_file.mesh_translation is None else original_segment.mesh_file.mesh_translation * scale_factor,
        )
    ...
    def scale_imu(self, original_imu: InertialMeasurementUnitReal, scale_factor: np.ndarray) -> InertialMeasurementUnitReal:
