...
class BiomodModelParser:
    def __init__(self, filepath: str):
        ...
        self.imus: dict[str, InertialMeasurementUnitReal] = {}
        ...
        while token_index < len(tokens):
            ...
            if current_component is None:
                if token == "version":
                    ...
                    rt_in_matrix_default = biomod_version < 4  # True for version 3 or less, False for version 4 or more
                elif token == "segment":
                ...
                if token == "endsegment":
                    ...
                    current_component.segment_coordinate_system = scs
                    ...
                    current_component.mesh.positions = np.concatenate(
                        (current_component.mesh.positions, position), axis=1
                    )
                ...
                if token == "endimu":
                    ...
                    self.imus[current_component.name] = current_component
                    current_component = None
                elif token == "parent":
                    current_component.parent_name = _read_str(next_token=next_token)
                elif token == "rt":
    ...
    def to_real(self) -> BiomechanicalModelReal:
        ...
        for segment in self.segments:
            model.segments.append(segment=segment)
...
def _read_str_vector(next_token: Callable, length: int) -> list[str]:
    return [_read_str(next_token=next_token) for _ in range(length)]


def _get_rt_matrix(next_token: Callable, current_rt_in_matrix: bool) -> SegmentCoordinateSystemReal:
    if current_rt_in_matrix:
        ...
        angle_sequence = _read_str_vector(next_token=next_token, length=3)
