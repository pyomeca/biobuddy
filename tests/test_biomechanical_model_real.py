...
def test_remove_muscle_group():
    ...
    with pytest.raises(
        AttributeError, match="The item named non_existent_muscle cannot be removed because it it not in the list."
    ):
...
def test_remove_ligament():
    ...
    model.add_ligament(
        LigamentReal(
            ...
            origin_position=ViaPointReal(
                name="lig_origin", parent_name="parent", position=np.array([[0.0], [0.0], [0.0], [1.0]])
            ),
            insertion_position=ViaPointReal(
                name="lig_insertion", parent_name="child", position=np.array([[0.0], [0.0], [0.0], [1.0]])
            ),
    ...
    # Test removing non-existent muscle group
    with pytest.raises(
        AttributeError, match="The item named non_existent_ligament cannot be removed because it it not in the list."
    ):
...
def test_segment_coordinate_system_in_local():
    ...
    model.segments["parent"].segment_coordinate_system.scs = RotoTransMatrix.from_euler_angles_and_translation(
        "xyz", angles=np.array([0.3, 0.2, 0.1]), translation=np.array([0.5, 0.0, 0.5])
    )
    model.segments["child"].segment_coordinate_system.scs = RotoTransMatrix.from_euler_angles_and_translation(
        "xyz", angles=np.array([0.1, 0.2, 0.3]), translation=np.array([0.25, 0.5, 0.75])
    )
    ...
    npt.assert_almost_equal(
        ...
        np.array(
            [
                [0.93629336, -0.28962948, 0.19866933, 0.25],
    ...
    npt.assert_almost_equal(
        new_scs_local.rt_matrix,
        np.array(
            [
                [0.986572, -0.16165639, 0.02330031, -0.20673285],
