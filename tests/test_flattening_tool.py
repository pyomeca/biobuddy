...
def test_check_model():
    ...
    with pytest.raises(ValueError, match="Segment root has a rotated coordinate system. Flattening is only possible if all segment coordinate systems are aligned."):
        flattening_tool._check_model()
...
def test_modify_jcs():
    ...
    npt.assert_almost_equal(
        flattening_tool.flattened_model.segments[2].segment_coordinate_system.scs.translation,
    ...
    npt.assert_almost_equal(
        flattening_tool.original_model.segments[2].segment_coordinate_system.scs.translation,
        np.array([0.0, 0.0, 1.0])
    )
...
def test_modify_markers():
    ...
    # Before modification
    npt.assert_almost_equal(
        flattening_tool.flattened_model.segments[1].markers[0].position[:3].reshape((3, )),
    ...
    # Original model should be unchanged
    npt.assert_almost_equal(
        flattening_tool.original_model.segments[1].markers[0].position[:3].reshape((3, )),
        np.array([0.1, 0.2, 0.3])
    )
...
def test_modify_contacts():
    ...
    # Before modification
    npt.assert_almost_equal(
        flattening_tool.flattened_model.segments[1].contacts[0].position[:3].reshape((3, )),
    ...
    # Original model should be unchanged
    npt.assert_almost_equal(
        flattening_tool.original_model.segments[0].contacts[0].position[:3].reshape((3, )),
        np.array([0.05, 0.2, 0.15])
    )
...
def test_modify_imus():
    ...
    with pytest.raises(NotImplementedError, match="This feature was never tested. If you encounter this error, please contact the developers."):
...
def test_flatten():
    ...
    npt.assert_almost_equal(
        flattening_tool.flattened_model.segments[2].markers[0].position[:3].reshape((3, )),
    ...
    npt.assert_almost_equal(
        flattening_tool.flattened_model.segments[2].contacts[0].position[:3].reshape((3, )),
        np.array([-0.05,  0.5 ,  0.35])
    )
    npt.assert_almost_equal(
        flattening_tool.flattened_model.muscle_groups[0].muscles[0].origin_position.position[:3].reshape((3, )),
        np.array([0.0 , 0.1, 0.0 ])
    ...
    npt.assert_almost_equal(
        flattened_model.segments[2].markers[0].position[:3].reshape((3, )),
        np.array([0.4, 0.0, 0.6])
    )
    npt.assert_almost_equal(
        flattened_model.segments[2].inertia_parameters.center_of_mass[:3].reshape((3, )),
        np.array([0.0, 0.0, 0.0])
    )
    npt.assert_almost_equal(
        flattened_model.segments[2].contacts[0].position[:3].reshape((3, )),
        np.array([-0.05,  0.0 ,  0.35])
    )
    npt.assert_almost_equal(
    ...
    npt.assert_almost_equal(
        flattened_model.muscle_groups[0].muscles[0].via_points[0].position[:3].reshape((3, )),
        np.array([0.2, 0.0, 0.4])
    )
    npt.assert_almost_equal(
    ...
    original_muscle_length = flattening_tool.original_model.muscle_tendon_length(flattening_tool.original_model.muscle_groups[0].muscles[0].name)
    flattened_muscle_length = flattening_tool.flattened_model.muscle_tendon_length(flattening_tool.original_model.muscle_groups[0].muscles[0].name)
