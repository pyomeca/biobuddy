...
def test_append_with_method_name():
    """Test appending an item with a method name."""
    named_list = NamedList()
    named_list._append(MethodNamedObject("method_item"))
    assert len(named_list) == 1
    assert named_list.keys() == ["method_item"]
...
def test_mixed_name_types():
    """Test a NamedList with mixed name types (property, method, attribute)."""
    named_list = NamedList()
    named_list._append(SimpleNamedObject("attr_item"))
    named_list._append(PropertyNamedObject("prop_item"))
    named_list._append(MethodNamedObject("method_item"))
    
    assert len(named_list) == 3
    assert named_list.keys() == ["attr_item", "prop_item", "method_item"]
    assert named_list["attr_item"].name == "attr_item"
    assert named_list["prop_item"].name == "prop_item"
    assert named_list["method_item"].name() == "method_item"
